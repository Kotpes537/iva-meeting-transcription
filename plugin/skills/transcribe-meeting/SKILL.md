...
description: Use when the owner sends a supported audio meeting recording in the private Telegram chat and asks for transcription, meeting notes, or a Word report.
---
...
1. For an audio file already saved under `vault/attachments/`, call `meeting_transcription__queue_meeting` with its path relative to `vault/attachments/`. Supported extensions are `.m4a`, `.mp3`, `.wav`, `.flac`, `.ogg`, `.oga`, `.opus`, `.aac`, `.webm` and audio `.mp4`.
