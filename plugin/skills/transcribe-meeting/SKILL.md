...
3. Choose `mode` from the owner's caption: `bullets` for only meeting notes in Word, `transcript` for only the full transcript in Word, `both` for both in one Word file. Default to `both`. The plugin sends short bullets as a Telegram message in every mode.
