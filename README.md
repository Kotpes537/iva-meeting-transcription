...
- На тестовой записи M4A (25,7 МБ, 26 минут) проверены распознавание, автоматические итоги и создание всех трёх вариантов Word. Сама запись и результаты обработки в репозиторий не входят.
- Unit-тесты и сборка плагина проходят. На действующей Iva плагин ещё не установлен; приём файла одним сообщением в Telegram пока не проверен сквозным тестом.
...
- `Итоги встречи. Word: только основные буллеты.`
- `Расшифруй встречу. Word: только транскрипт.`
- `Итоги и полный транскрипт в одном Word.`

Ива сообщит, что задача принята. После обработки плагин ответит на исходное сообщение тезисами и пришлёт Word отдельным файлом. В каждом режиме тезисы приходят в Telegram. Дата берётся из имени файла или метаданных; если данных нет, это будет прямо указано.
